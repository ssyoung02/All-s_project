...
<head>
    ...
    <script>
        ...
        const studyIdx = ${studyGroup.studyIdx};
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const studyIdx = ${studyGroup.studyIdx}; // 스터디 그룹 ID 가져오기
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                timeZone: 'UTC',
                initialView: 'dayGridMonth', // 초기 뷰 설정 (월)
                editable: true, // 수정 가능 여부
                selectable: true, // 날짜 선택 가능 여부
                eventOverlap: false, // 이벤트 겹침 방지
                events: function (fetchInfo, successCallback, failureCallback) {
                    $.ajax({
                        url: "${root}/calendar/teamEvents/" + studyIdx, // 팀 일정 가져오는 엔드포인트
                        type: "GET",
                        headers: {
                        ...
                        success: function (response) {
                            const events = response.map(event => ({
                                id: event.teamScheduleIdx, // TeamCalendar의 PK 사용
                                title: event.title,
                                start: event.start,
                                end: event.end,
                                ...
                                extendedProps: { // 추가 데이터 전달
                                    ...
                                    teamCalIdx: event.teamCalIdx,
                                    studyIdx: event.studyIdx
                                }
                            }));
            ...
            $(document).on('click', '#deleteButton', function () {
                ...
                if (confirm("정말로 이 일정을 삭제하시겠습니까?")) {
                    $.ajax({
                        url: "${root}/calendar/deleteTeamSchedule/" + eventId,
                        type: "POST",
                        headers: {
                            "${_csrf.headerName}": "${_csrf.token}"
                        },
            ...
            function openModal(modalId, titleText, submitButtonText, formId, eventData = null) {
                ...
                if (form && formId === 'addScheduleForm') {
                    ...
                    const teamScheduleIdx = eventData.id; // teamScheduleIdx 사용
                    //수정할 데이터 불러오기
                    $.ajax({
                        url: "${root}/calendar/teamEvent/" + teamScheduleIdx,
                        type: "GET",
                        headers: {
                        ...
                        success: function (response) {
                            if (response) {
                                $('#' + formId + ' #editTeamScheduleId').val(response.teamScheduleIdx); // 수정 모달의 hidden input에 scheduleIdx 설정
        ...
        $(document).on('submit', '#addScheduleForm, #editScheduleForm', function (event) {
            ...
            const url = isEdit ? "${root}/calendar/updateTeamSchedule" : "${root}/calendar/addTeamSchedule";
            ...
            formData += "&studyIdx=" + ${studyGroup.studyIdx}; // studyIdx 값 추가
            $.ajax({
                url: url,
                type: "POST",
